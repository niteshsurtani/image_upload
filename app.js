...
app.post('/imageupload', function (req, res){
  ...
  form.parse(req, function(err, fields, files) {
    ...
    // cloudinary.image("limit.png", { alt: "Sample Image" })
    res.end();
  });
  ...
  form.on('end', function(fields, files) {
    ...
    if(height >= 1024 && width >= 1024){
      ...
      // var imagefile = new_location + file_name;
      // var result = smartCrop.crop(imagefile, {width: 100, height: 100}, function(result){console.log(result);});

      // fs.copy(result, new_location + 'cropped', function(err) {  
      //   if (err) {
      //     console.error(err);
      //   } else {
      //     console.log("success!")
      //   }
      // });
      // gm(file).resize(200, 200).write(writeStream, function (err) {
      //   if (err) {
      //     console.error(err);
      //   }
      // });
